...
void exec(char* filename) {
	...
	if (header->c64_marker == O65_NON_C64 &&
		...
		cprintf("Code Base: %x\n", code_base);
		cprintf("Code Length: %x\n", code_len);
		cprintf("Data Base: %x\n", data_base);
		cprintf("Data Length: %x\n", data_len);
		...
		// this copies the code segment, but we need to find where the data segment is
		fat_read_bytes(code_base, cluster, offs, code_len);

		// so we know the number of bytes read (code_len), so if we add that to the original
		// offset then that is 
		...
		// bam so now we need to go past the underfined references list and the relocation
		// tables. We can skip the first by getting the number of undefined references

		offs += data_len;
		fat_read_bytes((uint8_t*)&num_undef_ref, cluster, offs, 2);
		cprintf("Number of undefined references: %d\n", num_undef_ref);

		if (num_undef_ref) {
			cprintf("Error! undefined reference encountered!\n");
			for(;;);
		}
		...
		// that got most of it but we need to look for the exported globals list, which will
		// have the vectors.
